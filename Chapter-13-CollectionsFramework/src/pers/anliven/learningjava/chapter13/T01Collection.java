...
Collection接口的子接口
- List接口: 序列，有序的集合，元素依据索引位置排序，可以包含重复的元素，按照索引可以精确操作元素。
- Queue接口 ：队列，存储的元素是排列有序，可以重复。不常用。
- Set接口: 集，无序的集合，不能包含重复的元素。实现类TreeSet能对元素按特定方式排序。
...
常用工具类
- java.util.Arrays和java.util.Collections，包含各种常用操作方法。
